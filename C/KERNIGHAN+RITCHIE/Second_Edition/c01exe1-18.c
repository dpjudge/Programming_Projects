...
main()
{
  ...
  while ((trimlen = initlen = Getline(line, MAXLINE)) > 0) {
    trimlen = trimline(line, initlen);
    if (trimlen > max) {
      max = trimlen;
    ...
    if (trimlen < initlen)
      printf("Initial line trimmed from %d to %d characters by removal of trailing white space\n\n",initlen,trimlen);
